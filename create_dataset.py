...
import os
cwd = os.getcwd()
dress_patterns_df = pd.read_csv(cwd+'\dress_patterns.csv')
...
os.mkdir('dataset_category_test')

for cat in category:
    print(cat)
    os.mkdir('dataset_category/'+cat)
    os.mkdir('dataset_category_test/'+cat)
...
for i in range(len(dress_patterns)):
  ...
  try:
    ...
    if(i%5==0):
      open('dataset_category_test/'+category+'/'+str(unit_id)+'.jpg', 'wb').write(r.content)
    else:
